...
<task type="auto">
  ...
  <action>
    ...
    **4. Create client/src/main/p2p/node.ts:**
    ...
    - The server PeerId needs to be fetched — add it to the server info endpoint response or fetch via a new REST endpoint `GET /api/p2p/info` that returns `{ peer_id, multiaddr, libp2p_port }`. Alternatively, include it in the existing `ServerInfo` WS response. For simplicity, query it via the existing WS connection.
