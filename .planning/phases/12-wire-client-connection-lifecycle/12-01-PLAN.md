...
must_haves:
  truths:
    ...
    - "Both flows store JWT in auth.ts module scope so connectWebSocket() can use it from /app"
    - "WsClient first reconnect attempt is immediate (0ms delay), then exponential backoff to 30s cap"
  artifacts:
