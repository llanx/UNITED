...
must_haves:
  truths:
    ...
    - "New npm dependencies installed: react-markdown, remark-gfm, rehype-highlight, highlight.js, emoji-picker-react, date-fns, @tanstack/react-virtual"
  artifacts:
...
```bash
cd client && npm install react-markdown remark-gfm rehype-highlight highlight.js emoji-picker-react date-fns @tanstack/react-virtual
```
...
```
  ...
  <verify>
`cd client && npm ls react-markdown remark-gfm rehype-highlight highlight.js emoji-picker-react date-fns @tanstack/react-virtual` -- all 7 packages listed.
Check that `client/src/main/ipc/channels.ts` has no TypeScript errors: `cd client && npx tsc --noEmit --pretty 2>&1 | head -20` (may have other errors from Phase 3 incomplete, but no errors in channels.ts itself).
  ...
  <done>
Seven new npm packages installed (react-markdown, remark-gfm, rehype-highlight, highlight.js, emoji-picker-react, date-fns, @tanstack/react-virtual). IPC channel constants extended with 14 new channels for chat, reactions, presence, last-read, and push events.
  </done>
</task>
...
- `CHAT_SEND` handler: Takes { channelId, content, replyToId? }. Sends POST to server `POST /api/channels/{channelId}/messages` with body `{ content, reply_to_id }` and JWT auth token. The server (plan 01) handles sender identification from JWT, assigns server_sequence, persists, and broadcasts. Returns the server response (ChatMessage with server_sequence, id, timestamp) so the renderer can confirm delivery.
...
<verification>
- npm dependencies installed and importable (7 packages including @tanstack/react-virtual)
...
<success_criteria>
1. Seven npm packages installed (react-markdown, remark-gfm, rehype-highlight, highlight.js, emoji-picker-react, date-fns, @tanstack/react-virtual)
